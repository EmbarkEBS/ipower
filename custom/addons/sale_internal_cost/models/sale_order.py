        ...
        for order in self:
            ...
            if total_charge > 0:
                if charge_line:
                    ...
                    # Create new line at the bottom. FIX: Use 'tax_id' (plural)
                    # Use a list of commands (0, 0, values) to add the line
                    new_line_vals = {
                        'name': 'Internal Charges (Freight, Duty, Misc)',
                        'product_uom_qty': 1.0,
                        'price_unit': total_charge,
                        'tax_id': [(6, 0, [])], # No taxes applied to this line
                        'x_is_charge_line': True,
                    }
                    order.order_line = [(0, 0, new_line_vals)]
            elif charge_line:
                # Remove the line if all charges are cleared
